...
async function createVideo() {
    ...
    try {
        ...
        await execPromise(`ffmpeg -y -i "${backgroundPath}" -filter_complex "[0:v]reverse[r];[0:v][r]concat=n=2:v=1:a=0,format=yuv420p[outv]" -map "[outv]" -an -c:v libx264 -preset veryfast -crf 18 "${tempPaths.pingPong}"`);
        ...
        const command = `ffmpeg -y -stream_loop -1 -i "${tempPaths.pingPong}" -i "${foregroundPath}" -loop 1 -i "${tempPaths.title}" -loop 1 -i "${tempPaths.info}" -loop 1 -i "${tempPaths.ikOnkar}" -filter_complex "${filter}" -map "[outv]" -map 1:a? -t ${VIDEO_DURATION} -c:v libx264 -preset medium -crf 22 -pix_fmt yuv420p -c:a aac -b:a 128k -movflags +faststart "${outputPath}"`;
